...
def punctuation_density(segments) -> float:
    ...
    span = max(
        1.0,
        float((last_end if last_end is not None else segments[-1].get("start", 0)))
        - float(segments[0].get("start", 0)),
    )
...
def paragraphs(segments, gap: float = PARAGRAPH_GAP) -> list:
    ...
    for i, s in enumerate(segs):
        ...
        pause = (
            ((float(nxt["start"]) - float(s["end"])) if nxt and s.get("end") is not None else 0.0)
            if nxt
            else 99.0
        )
...
def _join(group) -> dict:
    ...
    return {
        ...
        "text": re.sub(r"\s+", " ", " ".join((g.get("text") or "").strip() for g in group)).strip(),
    }
...
def to_markdown(paras, video_id: str = "", title: str = "") -> str:
    ...
    for p in paras:
        ...
        if video_id:
            lines.append(
                "**[%s](https://www.youtube.com/watch?v=%s&t=%ds)**  "
